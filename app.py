...
def toggle_controls(disabled=True):
    ...
    for name, component in ui_e.items():
        if name == "abort_button":
            updates[name] = gr.update(interactive=not disabled)  # Opposite behavior for abort
        else:
...
def get_model_info():
    ...
    dtype = str(next(model.parameters()).dtype)  # Get precision mode
    return model_name, device, f"{vram_used} / {vram_total}", dtype, str(model.config)  # Last one dumps model config
...
# Following the official snippet from:
...
def generate_caption(media_path, prompt, max_tokens, summary_mode=False, one_sentence_mode=False, resolution_mode="auto"):
    ...
    # Enable this if you want to debug the messages
    # print(json.dumps(serialize_for_debug(messages), indent=2))
...
def process_folder(folder_path, prompt, skip_existing, max_tokens, summary_mode, one_sentence_mode, retain_preview, resolution_mode):
    ...
    control_updates = finish_process()
    yield (
        f"✅ Processing complete! , processed {processed_media} media in {elapsed_str}, skipped {skipped_media} media. Failed to process {failed_media} media (inaccessible, unknown or broken file)", # status_output
        last_media_to_show,
        last_media_name_markdown,
        last_caption,
        None,
...
with gr.Blocks(theme=gr.themes.Base(), css=css) as iface: # type: ignore
    ...
    gr.Markdown("A simple media caption generator for images and video using **[Qwen2.5-VL-7B-Instruct](https://huggingface.co/Qwen/Qwen2.5-VL-7B-Instruct)**")
